...
class HybridSegModel_contour(nn.Module):
    def __init__(self, in_channels = 1, out_channels = 2, output_size = 224, layers_num = 5):
        super(HybridSegModel_contour, self).__init__()
        ...
        self.decoder_1 = Decoder(in_channels = decoder_in_channels, out_channels = decoder_out_channels, output_size = output_size, layers_num = decoder_layers)
        self.decoder_2 = Decoder(in_channels = decoder_in_channels, out_channels = decoder_out_channels, output_size = output_size, layers_num = decoder_layers)
    ...
    def forward(self, x):
        ...
        out_1 = self.decoder_1(pmfs_out)
        out_2 = self.decoder_2(pmfs_out)

        attention_upsample_cat = torch.cat(attention_upsample, axis = 1)
        out_cat_1 = torch.cat([attention_upsample_cat, out_1], axis = 1)
        out_cat_2 = torch.cat([attention_upsample_cat, out_2], axis = 1)

        out_1 = self.final_conv(out_cat_1)
        out_2 = self.final_conv(out_cat_2)
