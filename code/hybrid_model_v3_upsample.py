...
class Decoder(nn.Module):
    def __init__(self, in_channels=60, out_channels=1, output_size=224, layers_num=4):
        ...
        self.all_ups = nn.ModuleList([
            ...
            nn.Upsample(scale_factor=2, mode='nearest') for _ in range(layers_num)
        ])
            

        self.all_convs = nn.ModuleList([
            nn.Sequential(nn.Conv2d(channels[i], channels[i + 1], kernel_size=3, padding=1), nn.ReLU(inplace=True)) for i in range(layers_num)
        ])
