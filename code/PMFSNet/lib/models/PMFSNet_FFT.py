...
from PMFSNet.lib.models.modules.ConvBlock import ConvBlock
from PMFSNet.lib.models.modules.LocalPMFSBlock import DownSampleWithLocalPMFSBlock
from PMFSNet.lib.models.modules.GlobalPMFSBlock import GlobalPMFSBlock_AP_Separate


class PMFSNet_FFT(nn.Module):
    def __init__(self, in_channels=1, out_channels=35, dim="3d", scaling_version="TINY",
        ...
        super(PMFSNet_FFT, self).__init__()
    ...
    def low_pass(self, x):
        # FFT the input
        x_fft = torch.fft.fft2(x)  # [B, C, H, W] (for 2D), for 3D: use fftn
    
        # Shift the frequency domain (optional, for visualization)
        # x_fft_shifted = torch.fft.fftshift(x_fft)
    
        # Apply a simple low-pass filter
        # Create a binary mask with ones in the center (low freq) and zeros outside
        B, C, H, W = x.shape
        r = min(H, W) // 8  # radius of low-pass region
        mask = torch.zeros_like(x_fft, dtype=torch.bool)
    
        center_y, center_x = H // 2, W // 2
        y, x_ = torch.meshgrid(torch.arange(H), torch.arange(W), indexing='ij')
        dist = (y - center_y)**2 + (x_ - center_x)**2
        lp_filter = dist <= r**2
        mask[:, :, :, :] = lp_filter.to(x.device)
    
        # Apply the mask
        x_fft_filtered = torch.where(mask, x_fft, torch.zeros_like(x_fft))
    
        # Inverse FFT to go back to spatial domain
        x_filtered = torch.fft.ifft2(x_fft_filtered).real  # only real part
        return x_filtered
